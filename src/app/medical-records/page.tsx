...
import { MedicalRecordFormModal } from "@/components/medical-records/MedicalRecordFormModal";
    ...
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => setModalOpen(true)} className="gap-2">
          <Plus className="h-4 w-4" /> Write Record
        </Button>
      </div>

      {!data?.data.length ? (
        <EmptyState icon={FileText} title="No records yet" description="Medical records you create will appear here." />
      ) : (
      ...
      <MedicalRecordFormModal open={modalOpen} onOpenChange={setModalOpen} doctorId={doctorId} />
    </div>
  );
}
